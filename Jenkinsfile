pipeline {
  ...
  environment {
    BOT_NAME = 'internal_docs_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-internal_docs"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/internal_docs_bot/bin/internal_docs_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyInternalDocs.Release.create()'"
          $RELEASE_BIN eval 'BotArmyInternalDocs.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyInternalDocs.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyInternalDocs.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_internal_docs/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_internal_docs/releases/start_erl.data)
        fi
